...
error_counter = 0
error_projects = []
store_error_projects = []
cycle_staus = -1
starttime = None
endtime = None
rebuild_if = None
rebuild_starttime = None
rebuild_endtime = None
...
#
    ...
    def isPresent(self):
        ...
        if bool(error_flag):
            ...
            if bool(finish_flag):
                return True
    #rebuild error projects
    def rebuild_error_projects(self):
        global error_projects
        global rebuild_starttime
        global rebuild_endtime
        rebuild_starttime=strftime('%Y-%m-%d %H:%M:%S',time.gmtime())
        proj_id=[]
        for j in error_projects:
            proj_id.append(j[7:])
            print "j is %s, proj_id is %s" % (j,proj_id)

        print proj_id
        for i in proj_id:
             if i=='1':
                chk_proj=self.driver.find_elements_by_name("cbx")[0]
                chk_proj.click()
             elif i=='10':
                chk_proj=self.driver.find_elements_by_name("cbx")[1]
                chk_proj.click()
             else:
                chk_proj=self.driver.find_elements_by_name("cbx")[int(i)]
                chk_proj.click()
             print "i is %i" % int(i)
        #click rebuild button
        elem_rebuild= self.driver.find_elements_by_class_name("smallButton")[2]
        elem_rebuild.click()
        #check every one finish
        rebuild_finish_everyone = self.isEveryOneFinish()
        while rebuild_finish_everyone != 10:
            time.sleep(60)
            rebuild_finish_everyone = self.isEveryOneFinish()
        #check snapshot finish
        rebuild_running=True
        while  rebuild_running :
             time.sleep(60)
             rebuild_flag = self.isPresent()
             print 'rebuild flag is', rebuild_flag
             if rebuild_flag :
                 rebuild_endtime = strftime('%Y-%m-%d %H:%M:%S',time.gmtime())
                 time.sleep(10)
                 rebuild_screenshot_name ="./screenshot/" + self.param + "_cycle_after_rebuild.png"
                 self.driver.save_screenshot(rebuild_screenshot_name)
                 break
             ...
             if rebuild_flag == False:
                 ...
                 rebuild_screenshot_name ="./screenshot/" + self.param + "_cycle_after_rebuild.png"
                 self.driver.save_screenshot(rebuild_screenshot_name)
                 raise RuntimeError("some projects were also failed after rebuild in this cycle %s, failed projects is %s" % (self.param,error_projects))
                 break

                 print "---------flag is false------"
                 #if flag is None  go on loop
         #check if there also has failed projects
...
#                 f.close()
                 ...
                 #rebuild error project
                 if len(error_projects)!=0 :
                     store_error_projects=error_projects;
                     self.rebuild_error_projects()
                     rebuild_if=True
                 break
...
#                 f.close()
       ...
       # global starttime, endtime
        ...
        with open('./log/cycle_data.csv','ar+') as cvsfile:
                ...
                if error_counter != 0:
                    spamwriter.writerow([self.param, starttime,endtime,'error',store_error_projects,rebuild_if,rebuild_starttime,rebuild_endtime,'error',error_projects])
                else:
                    spamwriter.writerow([self.param, starttime,endtime,rebuild_if,rebuild_starttime,rebuild_endtime,'error',error_projects])
                    #spamwriter.writerow([self.param, starttime,endtime,rebuild_if,rebuild_starttime,rebuild_endtime])
        if error_counter !=0:
